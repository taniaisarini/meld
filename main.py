...
def flat_accuracy(preds, labels):
    ...
    # print("predicted: {} real: {}".format(pred_flat, labels_flat))
    # print(np.sum(pred_flat == labels_flat) )
    return np.sum(pred_flat == labels_flat) / len(labels_flat)


# Dataset
train_dataset = utils.create_dataset(df_train)
val_dataset = utils.create_dataset(df_dev)
test_dataset = utils.create_dataset(df_test)
...
def test():
    accuracy = 0
    with open('ouputs/1/output.csv', 'w', newline='') as csvfile:
...
if __name__ == "__main__":
    # test()
    utils.create_dataset(df_dev)
    # main()
    # w = utils.get_class_weights(df_train)
    # print(w)
    # print(utils.emotions)
    # a = torch.randn([1, 100, 512])
    # merged = utils.merged_strategy(a)
    # print(a.shape)
    # print(merged.shape)

# # test
# print('utterance: ', utterances[0])
